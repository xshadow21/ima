<?php include 'header.php' ?>

<!-- WRAPPER -->
<div id="wrapper"><!-- CAROUSEL -->
	<section class="container" style="margin-top: 80px;">
		<div class="row">
			<div class="col-md-6">
				<h2 style="color: #f68b1e;">Imprimantes / codeurs &agrave; jet d&rsquo;encre</h2>
				<hr class="sep" />
				<p class="emphase">Le syst&egrave;me d&rsquo;&eacute;tiquetage de vos boites joue un r&ocirc;le important tant pour la performance sur votre ligne de production que pour la qualit&eacute; de votre image de marque. Face au co&ucirc;t &eacute;lev&eacute; de l&rsquo;impression d&rsquo;&eacute;tiquettes traditionnelles etde ses limites sur le plan graphisme, l&rsquo;imprimante &agrave; jet d&rsquo;encre s&rsquo;av&egrave;re &ecirc;tre une solution de rechange int&eacute;ressante.</p>
				<p>Les imprimantes &agrave; jet d'encre haute r&eacute;solution de LM Manutention permettent la reproduction rapide et pr&eacute;cise de texte ou de graphique d'une qualit&eacute; impressionnante. Vous pourrez non seulement inscrire une date ou un code, mais aussi des codes-barres ou des logos sur les boites, quels que soient leur forme, leur taille et la quantit&eacute;.</p>
				<p>&Eacute;tant plus compactes que les imprimantes traditionnelles, les imprimantes &agrave; jet d&rsquo;encre peuvent &ecirc;tre int&eacute;gr&eacute;es optimalement &agrave; votre syst&egrave;me d&rsquo;emballage ou &agrave; votre ligne de production. Facile d&rsquo;utilisation, les codes &agrave; imprimer peuvent &ecirc;tre envoy&eacute;s &agrave; distance et les tests faits directement de votre site de production, les r&eacute;sultats en seront plus que satisfaisants.</p>
				<p>De plus, le codeur &agrave; jet d&rsquo;encre offre des possibilit&eacute;s de d&eacute;corer des bo&icirc;tes avec des logotypes de haute r&eacute;solution, d&rsquo;ajouter des offres sp&eacute;ciales, des promotions, etc. Le processus productif est ainsi plus flexible et dynamique, plus adapt&eacute; &agrave; la demande du march&eacute;.</p>
				<p>Plus besoin de confier ce travail &agrave; des fournisseurs externes et d&rsquo;entasser des boites pr&eacute;-imprim&eacute;es. Fini la d&eacute;pendance ! Avec pour r&eacute;sultat, que votre syst&egrave;me de codage &agrave; jet d&rsquo;encre contribuera &agrave; r&eacute;duire les co&ucirc;ts et les d&eacute;lais de livraison.</p>
				&nbsp; <br /> <a class="BlocOrangeButton pull-left" href="https://www.lmmanutentions.com/fr/demander-une-soumission.php"> Demander une<br /><strong>soumission</strong> </a>
				<div class="BlocOrangeButton pull-right">Information<br /><strong>450-226-8777</strong></div>
      </div>
			<div class="col-md-6">
				<div class="owl-carousel controlls-over" data-plugin-options="{">
					<div><img class="img-responsive" src="/images/emballage/ink1.jpg" alt="" /></div>
					<div><img class="img-responsive" src="/images/emballage/ink2.jpg" alt="" /></div>
				</div>
				<div class="Boite" style="color: #fff;">
					<h3>Nous travaillons <strong>pour vous</strong></h3>
					<p>Notre responsabilit&eacute; est de guider notre client&egrave;le dans leur choix en fonction de leurs objectifs, besoins, conditions d'op&eacute;ration et de leur budget. Et ce, jusqu'&agrave; la mise en place du syst&egrave;me op&eacute;rationnel, durable et efficace.</p>
				</div>
			</div>
		</div>
      <?php include 'emballage-menu.php' ?>
	</section>
</div>
<!-- /CAROUSEL -->
<p>&nbsp;</p>
<!-- /WRAPPER -->
<?php include 'footer.php' ?>