...
function _topNav() {
	...
	if(jQuery(window).width() > 1006) {
		...
		// #topHead Fixes
		window._headHeight		= 131;
		window._headHeightSmall	= 80;
		...
		function _topNavCalibrate() {
			...
			if(jQuery("#topHead").length > 0) {
				...
				if(jQuery(window).width() < 1006) {
					...
					// Mobile shadow
					jQuery('#header_shadow').css({"top":"50px"});
				} else {
					jQuery('#wrapper').css({"margin-top":"0px"});

					// From mobile back to desktop - do not leave the shadow behing
					if(jQuery(window).scrollTop() > 3) {
						jQuery('#header_shadow').css({"top":"110px"});
					} else {
						jQuery('#header_shadow').css({"top":"130px"});
					}
				}
	...
	function topMain() {
		...
		if(window.topNavSmall === false && _scrollTop > 0) {
			...
			jQuery('#topNav').stop().animate({ 'height':'110px'},400);
			
			/* Nav margin (on scroll) */
			jQuery('header#topNav div.nav-main-collapse').stop().animate({ 'margin-top':'6px'},400);
			
			/* Others param (on scroll) */
		...
		if(window.topNavSmall === true && _scrollTop < 3) {
			...
			/* Nav height (on scroll back top) */
			jQuery('#topNav').stop().animate({ 'height':'131px'},400);
			
			/* Nav margin (on scroll back top) */
			jQuery('header#topNav div.nav-main-collapse').stop().animate({ 'margin-top':'16px'},400);
			
			/* Others param (on scroll back top) */
			jQuery('header#topNav button').stop().animate({ 'margin-top':'8px'},100);
			jQuery('header#topNav a.logo').stop().animate({ 'line-height':'100px', 'margin-top':'0'},400);
