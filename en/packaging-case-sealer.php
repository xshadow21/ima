<?php include 'header.php' ?>

<!-- WRAPPER -->
<div id="wrapper"><!-- CAROUSEL -->
  <section class="container" style="margin-top: 80px;">
    <div class="row">
      <div class="col-md-6">
        <h2 style="color: #f68b1e;">CASE SEALING SYSTEM - CASE SEALER MACHINE</h2>
        <hr class="sep" />
        <p class="emphase">Do you need only a sealing system or a simple sealer machine to complete your product's packaging line? LM Manutentions has the solution for you.</p>
        <p>Our locking and sealing boxes systems are designed to meet your needs of packaging and of production. These cartoners, also known as packing, can operate uniform size cardboard boxes and those with random different formats. They are robust, efficient, fast and adaptable to all types of production line and packaging.</p>
        <p>The machine allows to apply, automatically or semi-automatically, tape or glue on the bottom and top of the case to close and seal all flaps of the cardboard boxes. You will save huge time and will ensure a perfect and solid box closure, ready to leave the warehouse to reach your customers.</p>
        <p>LM Manutentions bonding and closing cardboard boxes systems can fit all types of demands from different industries; including food industries that requires higher health checks.</p>
        <p>We offer high-quality systems that achieve a high performance level. They are facile to operate (user friendly) and easy to maintain. LM Manutentions will train your staff to obtain an optimal use and maintenance of the sealing system or machine.</p>
          <br /><strong><a class="BlocOrangeButton pull-left" href="/en/request-a-quote">Request<br>a Quote</a></strong>
        <div class="BlocOrangeButton pull-right">Information<br /><strong>450-226-8777</strong></div>
        <br/>
				<hr class="sep" />  
      </div>
      <div class="col-md-6">
        <div class="owl-carousel controlls-over" data-plugin-options="{">
          <div><img class="img-responsive" src="/images/emballage/case-sealer1.jpg" alt="" /></div>
          <div><img class="img-responsive" src="/images/emballage/case-sealer1.jpg" alt="" /></div>
        </div>
        <div class="Boite" style="color: #fff;">
          <h3>WE WORK <strong>FOR YOU</strong></h3>
          <p>Our responsibility is to guide our clients in their choice based on their goals, their needs, their conditions of operation and their budget until the establishment of an operational, sustainable and efficient integrated handling or transport system.</p>
        </div>
      </div>
    </div>
      <?php include 'emballage-menu.php' ?>

  </section>
  <!-- /CAROUSEL --></div>
<!-- /WRAPPER -->
<?php include 'footer.php' ?>
